...
void ExtractionContainers::PrepareEdges(ScriptingEnvironment &scripting_environment)
{
    ...
    for (std::size_t i = 0; i < all_edges_list.size();)
    {
        ...
        auto min_forward = std::make_pair(std::numeric_limits<EdgeWeight>::max(), INVALID_PAYLOAD);
        auto min_backward = std::make_pair(std::numeric_limits<EdgeWeight>::max(), INVALID_PAYLOAD);
